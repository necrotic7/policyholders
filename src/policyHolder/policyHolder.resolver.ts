...
import { CreatePolicyHolderInput, UpdatePolicyHolderInput } from './schema/dto/policyHolder.input';
...
export class PolicyHolderResolver {
    ...
    async createPolicyHolder(
        @Args('policyholder', {description:'創建保戶表單'}) args: CreatePolicyHolderInput,
    ): Promise<schema|{}>{
        const result = await this.service.createPolicyHolder(args.name, args.introducer_code);
        return result;
    }

    @Mutation(_returns => schema, {description:'更新保戶'})
    async updatePolicyHolder(
        @Args('policyHolder', {description:'更新保戶表單'}) args: UpdatePolicyHolderInput,
    ): Promise<schema|{}>{
        const result = await this.service.updatePolicyHolder(args.code, args.name, args.introducer_code);
        return result;
    }
}
